...
import { Order, CajaStatus, Role, ClientDebt, ExtraMovement } from '../types';
import { 
  DollarSign, 
  Printer, 
  ArrowUpRight, 
...
export default function CajaPanel({ 
  ...
  const [localExtraMovements, setLocalExtraMovements] = useState<ExtraMovement[]>(() => {
    const saved = localStorage.getItem('rf_caja_extra_movements');
  ...
  useEffect(() => {
    if (propExtraMovements === undefined) {
      localStorage.setItem('rf_caja_extra_movements', JSON.stringify(localExtraMovements));
    }
  ...
  const openDebtSettleModal = (order: Order) => {
    setSelectedPendingOrder(order);
    const existing = clients.find(c => c.name.toLowerCase().trim() === (order.clientName || '').toLowerCase().trim());
    if (existing) {
  ...
  const handleAutoCreateClient = () => {
    ...
    const cleanName = selectedPendingOrder.clientName.trim();
    if (!cleanName) {
  ...
  return (
    <div id="caja-panel-container" className="space-y-6">
      ...
      <div id="caja-metrics-grid" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div id="metric-ventas-dia" className="bg-[#f2f9ff] p-4.5 rounded-xl border border-blue-200 shadow-xs flex flex-col justify-between">
          <div className="flex justify-between items-start">
            <span className="text-[10.5px] font-mono font-bold text-blue-800 uppercase tracking-wider">Ventas del Día</span>
            <span className="p-1 px-1.5 bg-blue-100 text-blue-700 rounded-lg text-xs font-bold font-mono">Total</span>
          </div>
          <div className="mt-3">
            <span className="text-2xl font-sans font-black text-blue-950">
              ${totalVendidoCalculated.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
            <p className="text-[10px] text-blue-700 mt-1 font-mono">
        ...
        <div id="metric-pago-entregado" className="bg-emerald-50/70 p-4.5 rounded-xl border border-emerald-200 shadow-xs flex flex-col justify-between">
          <div className="flex justify-between items-start">
            <span className="text-[10.5px] font-mono font-bold text-emerald-800 uppercase tracking-wider">Cobrado / Pagados</span>
            <span className="p-1 bg-emerald-100 text-emerald-800 rounded-lg">
              <Coins className="w-3.5 h-3.5" />
            </span>
          </div>
          <div className="mt-3">
            <span className="text-2xl font-sans font-black text-emerald-950">
              ${totalPedidosPagadosAmount.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
            <p className="text-[10px] text-emerald-700 mt-1 font-mono font-bold flex items-center gap-1">
        ...
        <div id="metric-pendientes-hoy" className="bg-amber-50/70 p-4.5 rounded-xl border border-amber-200 shadow-xs flex flex-col justify-between">
          <div className="flex justify-between items-start">
            <span className="text-[10.5px] font-mono font-bold text-amber-800 uppercase tracking-wider">Pedidos Pendientes</span>
            <span className="p-1 px-1.5 bg-amber-100 text-amber-800 rounded text-[10px] font-bold font-mono">Caja</span>
          </div>
          <div className="mt-3">
            <span className="text-2xl font-sans font-black text-amber-950">
              ${totalPedidosPendientesAmount.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
            <p className="text-[10px] text-amber-700 mt-1 font-mono leading-tight">
        ...
        <div id="metric-creditos-generados" className="bg-rose-50/70 p-4.5 rounded-xl border border-rose-200 shadow-xs flex flex-col justify-between">
          <div className="flex justify-between items-start">
            <span className="text-[10.5px] font-mono font-bold text-rose-800 uppercase tracking-wider">Créditos Generados</span>
            <span className="p-1 px-2 bg-rose-100 text-rose-800 rounded text-[9.5px] font-black font-mono">Fiado Hoy</span>
          </div>
          <div className="mt-3">
            <span className="text-2xl font-sans font-black text-rose-950">
        ...
        <div id="metric-dinero-lider" className="bg-purple-50 p-4.5 rounded-xl border border-purple-200 shadow-xs flex flex-col justify-between">
          <div className="flex justify-between items-start">
            <span className="text-[10.5px] font-mono font-bold text-purple-800 uppercase tracking-wider">Entregado a Líder</span>
            <span className="p-1 bg-purple-100 text-purple-700 rounded-lg">
              <Landmark className="w-3.5 h-3.5" />
            </span>
          </div>
          <div className="mt-3">
            <span className="text-2xl font-sans font-black text-purple-950">
              ${totalDineroEntregado.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </span>
            <p className="text-[10px] text-purple-600 mt-1 font-mono">
                        ...
                        <div className="flex items-end">
                          <button
                            ...
                            className={`w-full text-white font-sans font-extrabold text-xs py-2.5 rounded-lg text-center cursor-pointer transition-all active:scale-98 ${
                              selectedPendingOrderId ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-gray-300 cursor-not-allowed'
                            }`}
                          >
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div id="corte-de-caja-ticket" className="bg-white border-2 border-dashed border-gray-300 rounded-2xl shadow-sm p-5 space-y-4">
            ...
            <div id="ticket-financial-resumen" className="space-y-2 text-xs font-mono">
              ...
              <div className="flex justify-between items-center py-1 font-bold text-emerald-800 bg-emerald-50/50 px-2 rounded-md">
          ...
          <div id="caja-tips-card" className="bg-[#fcf9f8] border border-gray-200 p-4.5 rounded-2xl shadow-xs">
            <span className="text-[9.5px] font-mono font-bold uppercase text-amber-800 tracking-wider flex items-center gap-1">
              <BadgeAlert className="w-3.5 h-3.5 text-amber-600" />
              <span>Notas Operativas de Turnos</span>
            </span>
            <ul className="text-[11px] text-gray-600 list-disc pl-4 mt-2 space-y-1.5 leading-relaxed font-sans">
