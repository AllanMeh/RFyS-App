...
import { ClientDebt, Movement, STORES_LIST } from '../types';
import { 
  Search, 
  Plus, 
  Clock, 
...
export default function CreditosPanel({ 
  ...
  const [newClientForm, setNewClientForm] = useState({ 
    name: '', 
    phone: '', 
    branch: STORES_LIST[0] || 'Mesa (gente que llega al local)', 
    initialBalance: 0,
  ...
  const handleRegisterAbonoSubmit = (e: React.FormEvent) => {
    ...
    const formattedDate = new Date(actionDate + 'T12:00:00')
  ...
  const confirmFullPayment = () => {
    ...
    try {
      const savedCurrentUserStr = localStorage.getItem('rf_current_user') || localStorage.getItem('rf_user');
      if (savedCurrentUserStr) {
        adminName = savedCurrentUserStr;
      }
    } catch {}
  ...
  const handleDeleteDebtSubmit = (e: React.FormEvent) => {
    ...
    let adminName = 'Administrador';
    try {
      const savedUsersStr = localStorage.getItem('rf_users');
      if (savedUsersStr) {
        const users = JSON.parse(savedUsersStr) as any[];
  ...
  return (
    <div id="creditos-main-container" className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-[#fff4eb]/80 border border-amber-200 p-4 rounded-xl shadow-xs">
          <span className="text-[9.5px] font-bold font-mono text-amber-900 tracking-wider block">DEUDA TOTAL PENDIENTE</span>
        ...
        <div className="bg-rose-50/80 border border-rose-200 p-4 rounded-xl shadow-xs">
          <span className="text-[9.5px] font-bold font-mono text-rose-800 tracking-wider block">CLIENTES DEUDORES (ACTIVOS)</span>
          <p className="text-xl sm:text-2xl font-black text-rose-950 font-mono mt-1">
            {activeDebtorsCount} cuentas
          </p>
        </div>

        <div className="bg-emerald-50/80 border border-emerald-200 p-4 rounded-xl shadow-xs">
          <span className="text-[9.5px] font-bold font-mono text-[#006e0a] tracking-wider block">PAGADOS RECIENTES (3 DÍAS)</span>
          <p className="text-xl sm:text-2xl font-black text-[#004d07] mt-1 font-mono">
            {paidRecentCount} liquidados
          </p>
        </div>

        <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl shadow-xs">
          <span className="text-[9.5px] font-bold font-mono text-slate-700 tracking-wider block">CUENTAS ARCHIVADAS (CERRADAS)</span>
