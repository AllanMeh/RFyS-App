...
export default function BottomNav({ activeTab, setActiveTab, currentRole, readyCount, cocinaCount }: BottomNavProps) {
  ...
  return (
    <nav className="bg-white border-t border-gray-250 fixed bottom-0 left-0 right-0 z-50 shadow-lg px-2 sm:px-6 py-2.5 transition-all">
          ...
          return (
            <button
              ...
              className={`flex-1 flex flex-col items-center justify-center gap-1 text-center py-1 rounded-xl transition-all relative ${
                isActive 
                  ? 'bg-[#ffdcc3] text-[#904d00] font-bold p-1 max-w-[120px] scale-105' 
                  : 'text-gray-500 hover:text-gray-900 hover:bg-neutral-50 p-1 font-medium'
              }`}
            >
              <div className="relative">
                <Icon className={`w-5 h-5 ${isActive ? 'text-[#904d00]' : 'text-gray-500'}`} />
