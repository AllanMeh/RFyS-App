...
export default function DashboardPanel({ 
  ...
  const totalReceivables = clients.reduce((sum, c) => sum + (c.status === 'Activa' ? c.balance : 0), 0);
    ...
    <div className="space-y-6">
      ...
      <div className={`bg-gradient-to-r ${greetingBg} p-6 rounded-3xl shadow-md border border-amber-900/10 relative overflow-hidden transition-all duration-500`}>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div className="bg-white/95 p-1 rounded-2xl shadow-md border-2 border-amber-400 overflow-hidden w-16 h-16 shrink-0 flex items-center justify-center">
              <img 
                src={activeUserAvatar || "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=120&h=120&fit=crop"} 
                alt={activeUserName} 
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center px-1">
          <h2 className="text-sm font-black text-gray-900 tracking-wider uppercase flex items-center gap-1.5">
            <span className="w-1.5 h-3 bg-[#904d00] rounded"></span>
            Resumen Operativo del Día
          </h2>
          <span className="text-[11px] font-mono bg-yellow-100 text-yellow-905 px-2 py-0.5 rounded-full font-bold">
            ⚡ EN VIVO
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
          ...
          <button 
            ...
            className="bg-emerald-50 hover:bg-emerald-100/80 p-3.5 rounded-2xl border border-emerald-200 text-left transition-all hover:shadow-sm active:scale-97 cursor-pointer"
            id="kpi-ventas"
          >
            <span className="text-[10px] text-emerald-800 font-mono font-bold tracking-wider block">VENTAS</span>
            <p className="text-lg sm:text-xl font-sans font-extrabold text-emerald-900 mt-1 truncate">
              ${totalSalesThisDay.toLocaleString('es-MX', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}
            </p>
            <span className="text-[9px] text-emerald-700 font-bold block mt-1">Caja Chica</span>
          </button>

          {/* 2. Pedidos activos */}
          <button 
            onClick={() => onNavigate('Cocina')}
            className="bg-[#fff5eb] hover:bg-orange-100/80 p-3.5 rounded-2xl border border-orange-200 text-left transition-all hover:shadow-sm active:scale-97 cursor-pointer"
            id="kpi-activos"
          >
            <span className="text-[10px] text-orange-850 font-mono font-bold tracking-wider block">ACTIVOS</span>
            <p className="text-lg sm:text-xl font-sans font-extrabold text-orange-950 mt-1">
              {activeOrdersCount}
            </p>
            <span className="text-[9px] text-orange-700 font-bold block mt-1">En cola</span>
          </button>

          {/* 3. Pedidos pendientes */}
          <button 
            onClick={() => onNavigate('Cocina')}
            className="bg-amber-50 hover:bg-amber-100/80 p-3.5 rounded-2xl border border-amber-200 text-left transition-all hover:shadow-sm active:scale-97 cursor-pointer"
            id="kpi-pendientes"
          >
            <span className="text-[10px] text-amber-805 font-mono font-bold tracking-wider block">PENDIENTES</span>
            <p className="text-lg sm:text-xl font-sans font-extrabold text-amber-950 mt-1">
              {pendingOrdersCount}
            </p>
            <span className="text-[9px] text-amber-700 font-semibold block mt-1">Sin preparar</span>
          </button>

          {/* 4. Pedidos listos */}
          <button 
            onClick={() => onNavigate('Entregas')}
            className="bg-green-50 hover:bg-green-100/80 p-3.5 rounded-2xl border border-green-200 text-left transition-all hover:shadow-sm active:scale-97 cursor-pointer"
            id="kpi-listos"
          >
            <span className="text-[10px] text-green-800 font-mono font-bold tracking-wider block">LISTOS</span>
            <p className="text-lg sm:text-xl font-sans font-extrabold text-green-950 mt-1">
              {readyOrdersCount}
            </p>
            <span className="text-[9px] text-green-700 font-bold block mt-1">Para entrega</span>
          </button>

          {/* 5. Pedidos entregados */}
          <button 
            onClick={() => onNavigate('Entregas')}
            className="bg-slate-50 hover:bg-slate-100/80 p-3.5 rounded-2xl border border-slate-200 text-left transition-all hover:shadow-sm active:scale-97 cursor-pointer"
            id="kpi-entregados"
          >
            <span className="text-[10px] text-slate-700 font-mono font-bold tracking-wider block">ENTREGADOS</span>
            <p className="text-lg sm:text-xl font-sans font-extrabold text-slate-900 mt-1">
              {deliveredOrdersCount}
            </p>
            <span className="text-[9px] text-slate-500 font-semibold block mt-1">Completados</span>
          </button>

          {/* 6. Pedidos pagados */}
          <button 
            onClick={() => onNavigate('Caja')}
            className="bg-[#f0fdf4] hover:bg-emerald-50 p-3.5 rounded-2xl border border-emerald-150 text-left transition-all hover:shadow-sm active:scale-97 cursor-pointer"
            id="kpi-pagados"
          >
            <span className="text-[10px] text-[#006e0a] font-mono font-bold tracking-wider block font-black">PAGADOS</span>
            <p className="text-lg sm:text-xl font-sans font-extrabold text-[#004b04] mt-1">
              {paidOrdersCount}
            </p>
            <span className="text-[9px] text-[#006e0a] font-bold block mt-1">Con corte</span>
          </button>

          {/* 7. Clientes con crédito activo */}
          <button 
            onClick={() => onNavigate('Créditos')}
            className="bg-rose-50 hover:bg-rose-100/80 p-3.5 rounded-2xl border border-rose-200 text-left transition-all hover:shadow-sm active:scale-97 cursor-pointer col-span-2 sm:col-span-1"
            id="kpi-credito-clientes"
          >
            <span className="text-[10px] text-rose-800 font-mono font-bold tracking-wider block">FIADOS</span>
            <p className="text-lg sm:text-xl font-sans font-extrabold text-rose-950 mt-1">
              {activeCreditClientsCount}
            </p>
            <span className="text-[9px] text-rose-600 font-bold block mt-1">Por liquidar</span>
          </button>
      ...
      <div className="space-y-2">
        ...
        {urgentOrdersList.length === 0 ? (
          <div className="bg-emerald-50/50 p-4 rounded-2xl border border-dashed border-emerald-200 text-center text-xs text-emerald-950 font-sans flex items-center justify-center gap-2">
              ...
              return (
                <div 
                  ...
                  className={`p-4 rounded-2xl border transition-all ${
                    isHighUrgency 
                      ? 'bg-rose-50/70 border-rose-300 shadow-sm shadow-rose-100 animate-pulse' 
                      : 'bg-[#fffdf8] border-amber-200'
                  }`}
                >
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-mono font-extrabold tracking-wider ${
                      isHighUrgency ? 'bg-red-650 text-white animate-bounce' : 'bg-amber-100 text-amber-900'
                    }`}>
                  ...
                  <div className="mt-3">
                    ...
                    <p className="text-xs text-[#904d00] font-sans mt-0.5 font-bold">
                      📍 {getOrderBranch(order)}
                    </p>
                    <p className="text-xs text-gray-500 font-medium line-clamp-1 mt-1 font-mono">
                      {order.items.map(item => `${item.quantity}x ${item.product.name}`).join(', ')}
                    </p>
                  </div>

                  <div className="flex justify-between items-center mt-3 pt-3 border-t border-dashed border-gray-150">
                    <span className="text-xs text-gray-500 font-medium font-mono">
                      Registrado: {formatOrderTimeOnly(order.timestamp)}
                    </span>
                    
                    <span className={`text-xs font-mono font-bold ${isHighUrgency ? 'text-red-700' : 'text-amber-800'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-5 space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-gray-100">
              <div>
                <h3 className="font-sans font-black text-base text-gray-900 tracking-tight">PEDIDOS EN EJECUCIÓN</h3>
                <p className="text-[11px] text-gray-500 font-sans mt-0.5">Cola dinámica del día organizada por secuencia de arribo</p>
              </div>
              <button
            ...
            <div className="divide-y divide-gray-100 max-h-[420px] overflow-y-auto pr-1">
              {activeOrdersList.length === 0 ? (
                ...
                activeOrdersList.map((order) => {
                  ...
                  return (
                    <div key={order.id} className="py-3.5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 hover:bg-neutral-50/40 px-1 rounded-xl transition-colors">
                      <div className="flex items-start gap-3">
                        <div className="w-10 h-10 rounded-xl bg-amber-50 border border-amber-200 flex flex-col items-center justify-center text-amber-900 font-mono shrink-0">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="text-sm font-bold text-gray-950 font-sans leading-tight">
                              {order.clientName || 'Cliente de Caja General'}
                            </h4>
                            <span className="text-[10px] text-gray-400 bg-gray-100 px-1.5 py-0.2 rounded font-mono font-semibold">
                              {order.paymentMethod || 'Contado'}
                            </span>
                          </div>

                          <span className="text-xs text-gray-500 font-sans block mt-0.5">
                            🏠 sucursal: <strong className="text-gray-800">{sBranch}</strong>
                          </span>
          ...
          <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-5 space-y-4">
            <div>
              <h3 className="font-sans font-black text-base text-gray-900 uppercase tracking-tight">Accesos Rápidos del Sistema</h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3.5 pt-1">
              ...
              <button 
                onClick={() => onNavigate('POS')}
                className="p-3 bg-[#fff9f4] hover:bg-[#ffe3cc] border border-[#ffca99] rounded-2xl text-left group transition-all active:scale-95 cursor-pointer"
                id="quick-nav-pos"
              >
                <div className="w-8 h-8 rounded-lg bg-[#904d00] text-white flex items-center justify-center font-bold text-sm">
                  🛍️
                </div>
                <h4 className="text-xs font-extrabold text-gray-900 mt-2 font-sans group-hover:text-[#904d00]">Punto de Venta</h4>
                <p className="text-[10px] text-gray-500 mt-1 leading-snug">Levantar órdenes de jugos y snacks.</p>
              </button>

              <button 
                onClick={() => onNavigate('Cocina')}
                className="p-3 bg-[#f3faf3] hover:bg-emerald-100/[0.7] border border-emerald-200 rounded-2xl text-left group transition-all active:scale-95 cursor-pointer"
              ...
              >
                ...
                <h4 className="text-xs font-extrabold text-gray-900 mt-2 font-sans group-hover:text-[#006e0a]">Estación Cocina</h4>
                <p className="text-[10px] text-gray-500 mt-1 leading-snug">Preparar tickets o licuados al momento.</p>
              </button>

              <button 
                onClick={() => onNavigate('Entregas')}
                className="p-3 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-2xl text-left group transition-all active:scale-95 cursor-pointer"
              ...
              >
                ...
                <h4 className="text-xs font-extrabold text-gray-900 mt-2 font-sans group-hover:text-amber-700">Hub Entregas</h4>
                <p className="text-[10px] text-gray-500 mt-1 leading-snug">Despacho para choferes y repartidores.</p>
              </button>

              <button 
                onClick={() => onNavigate('Caja')}
                className="p-3 bg-slate-50 hover:bg-gray-150 border border-gray-250 rounded-2xl text-left group transition-all active:scale-95 cursor-pointer"
              ...
              >
                ...
                <h4 className="text-xs font-extrabold text-gray-900 mt-2 font-sans group-hover:text-amber-900">Arqueo de Caja</h4>
                <p className="text-[10px] text-gray-500 mt-1 leading-snug">Consultar cortes, fondos y retiras parciales.</p>
              </button>

              <button 
                onClick={() => onNavigate('Créditos')}
                className="p-3 bg-rose-50/55 hover:bg-rose-100 border border-rose-200 rounded-2xl text-left group transition-all active:scale-95 cursor-pointer"
              ...
              >
                ...
                <h4 className="text-xs font-extrabold text-gray-900 mt-2 font-sans group-hover:text-rose-700">Cuentas Fiadas</h4>
                <p className="text-[10px] text-gray-500 mt-1 leading-snug">Saldos vencidos de clientes de confianza.</p>
              </button>

              <button 
                onClick={() => onNavigate('Administración')}
                className="p-3 bg-neutral-50 hover:bg-neutral-100 border border-neutral-200 rounded-2xl text-left group transition-all active:scale-95 cursor-pointer"
                id="quick-nav-admin"
              >
                <div className="w-8 h-8 rounded-lg bg-slate-500 text-white flex items-center justify-center font-bold text-sm">
                  ⚙️
                </div>
                <h4 className="text-xs font-extrabold text-gray-900 mt-2 font-sans group-hover:text-gray-950">Administración</h4>
                <p className="text-[10px] text-gray-500 mt-1 leading-snug">Precios Sabritas, galletas y Menú del Día.</p>
              </button>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-5 space-y-4">
            <div>
              <h3 className="font-sans font-black text-sm text-gray-900 uppercase tracking-wider flex items-center gap-1.5 justify-between">
                <span>Ratios y Estadísticas</span>
                <TrendingUp className="w-4 h-4 text-emerald-600" />
              </h3>
              <p className="text-[10px] text-gray-500 font-sans">Análisis cuantitativo de flujos y carteras vigentes</p>
            </div>

            <div className="space-y-4">
              
              {/* Stat 1: Ventas del día */}
              <div className="p-3 bg-[#e8f5e9]/50 rounded-2xl border border-[#c8e6c9]/40 flex justify-between items-center">
                <div>
                  <span className="text-[10px] text-emerald-700 font-mono tracking-wider font-extrabold block">VENTADO HOY</span>
                  <span className="text-base font-extrabold text-emerald-950 block mt-0.5">${totalSalesThisDay.toFixed(2)}</span>
                </div>
                <DollarSign className="w-6 h-6 text-emerald-700" />
              </div>

              {/* Stat 2: Número de pedidos */}
              <div className="p-3 bg-slate-50 rounded-2xl border border-neutral-150 flex justify-between items-center">
                <div>
                  <span className="text-[10px] text-slate-500 font-mono tracking-wider font-extrabold block text-gray-500">CANT. PEDIDOS</span>
                  <span className="text-base font-extrabold text-slate-800 block mt-0.5">{totalOrdersCount} órdenes</span>
                </div>
                <ListOrdered className="w-5 h-5 text-slate-500" />
              </div>

              {/* Stat 3: Promedio por pedido */}
              <div className="p-3 bg-[#ffecc3]/20 rounded-2xl border border-orange-100 flex justify-between items-center">
                <div>
                  <span className="text-[10px] text-[#904d00]/80 font-mono tracking-wider font-extrabold block">PROMEDIO PEDIDO</span>
                  <span className="text-base font-extrabold text-amber-950 block mt-0.5">${(averageTicketPrice || 55.40).toFixed(2)}</span>
                </div>
                <Sparkles className="w-5 h-5 text-amber-600" />
              </div>

              {/* Stat 4: Clientes con crédito */}
              <div className="p-3 bg-red-50/50 rounded-2xl border border-rose-100 flex justify-between items-center">
                <div>
                  <span className="text-[10px] text-rose-700 font-mono tracking-wider font-extrabold block">CLIENTES FIADO</span>
                  <span className="text-base font-extrabold text-rose-950 block mt-0.5">{activeCreditClientsCount} deudores</span>
                </div>
              ...
              <div className="p-3 bg-amber-50/40 rounded-2xl border border-amber-100 flex justify-between items-center">
                <div>
                  <span className="text-[10px] text-amber-800 font-mono tracking-wider font-extrabold block">EN FILA ESPERA</span>
                  <span className="text-base font-extrabold text-amber-950 block mt-0.5">{pendingOrdersCount} sin atender</span>
                </div>
          ...
          <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-5 space-y-4">
            <div className="pb-2 border-b border-gray-100">
              <h3 className="font-sans font-black text-sm text-gray-900 uppercase tracking-wider flex items-center gap-1.5">
                <Activity className="w-4 h-4 text-[#904d00] animate-pulse" />
                Bitácora de Actividad Reciente
              </h3>
              <p className="text-[10px] text-gray-500 font-sans mt-0.5">Operaciones cronológicas registradas en toda la sucursal</p>
            </div>

            <div className="space-y-4 max-h-[360px] overflow-y-auto pr-1">
              {sortedActivities.length === 0 ? (
                <p className="text-center py-10 text-xs text-gray-400 font-sans">
                  No hay movimientos para desplegar en este ciclo.
                </p>
              ) : (
                <div className="space-y-4 relative before:absolute before:left-3.5 before:top-2 before:bottom-2 before:w-[1.5px] before:bg-orange-100">
                      ...
                      <div key={event.id} className="flex gap-3 relative z-10 text-xs items-start select-none">
                        ...
                        <div className="space-y-0.5">
                          <div className="flex justify-between items-center gap-2">
                            <span className="font-bold text-gray-950 font-sans leading-tight">
                              {event.title}
                            </span>
